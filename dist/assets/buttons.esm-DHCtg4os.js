...
var createElementInDocument = function(document$1) {
	return function(tag, props, children) {
		var el = document$1.createElement(tag);
		if (props != null) for (var prop in props) {
			var val = props[prop];
			if (val != null) if (el[prop] != null) el[prop] = val;
			else el.setAttribute(prop, val);
		}
		if (children != null) forEach(children, function(child) {
			el.appendChild(typeof child === "string" ? document$1.createTextNode(child) : child);
		});
		return el;
	};
};
...
var iframeURL = "https://unpkg.com/github-buttons@2.29.1/dist/buttons.html";
...
var offEvent = function(target, eventName, func) {
	/* istanbul ignore else: IE lt 9 */
	if (target.removeEventListener) target.removeEventListener(eventName, func, false);
...
var onceReadyStateChange = function(target, regex, func) {
...
var octicon = function(icon, height) {
	...
	var defaultHeight = height >= 24 && 24 in data[icon].heights ? 24 : 16;
...
var fetch = function(url, func) {
	...
	if (useXHR) {
		...
		onEvent(xhr, "load", function() {
			var data$1;
			try {
				data$1 = JSON.parse(this.responseText);
			} catch (error) {
				callback(error);
				return;
			}
			callback(this.status !== 200, data$1);
		});
		xhr.open("GET", url);
		xhr.send();
	} else {
		...
		var onloadend = function() {
			if (contentWindow._) contentWindow._({ meta: {} });
		};
...
var render$1 = function(root, options, func) {
	var createElement$1 = createElementInDocument(root.ownerDocument);
	var style = root.appendChild(createElement$1("style", { type: "text/css" }));
	...
	}, [createElement$1("span", {}, [options["data-text"] || ""])]);
	var widget = root.appendChild(createElement$1("div", { className: "widget" + (isLarge ? " widget-lg" : "") }, [btn]));
	...
	fetch.call(this, apiBaseURL + api, function(error, json) {
		if (!error) {
			var data$1 = json[property];
			widget.appendChild(createElement$1("a", {
				className: "social-count",
				href: json.html_url + href,
				rel: "noopener",
				target: "_blank",
				"aria-label": data$1 + " " + property.replace(/_count$/, "").replace("_", " ").slice(0, data$1 < 2 ? -1 : void 0) + " on GitHub"
